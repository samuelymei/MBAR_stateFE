...
!      this%reducedEnergies(:) = this%reducedEnergies(:) - avgEner
    ...
    subroutine computePMF(this, setnbins, setbinmin, setbinmax, setiGaussSmooth)
      use io_m
      use precision_m
      use bin_m
      use MBAR_m
      ...
      implicit none
      class(reducedHamiltonian_t) :: this
      integer(kind=4), intent(in) :: setnbins
      real(kind=fp_kind), intent(in) :: setbinmin, setbinmax
      integer(kind=4), intent(in), optional :: setiGaussSmooth
      ...
      call coordBins%initbins(setbinmin, setbinmax, setnbins)
...
! probabilities of some low energy configurations
      if(present(setiGaussSmooth) .and. setiGaussSmooth > 0) then
        ...
        do IndexB = 1, coordBins%nbins
          ...
          write(6,'(1X,A,I5,A,I8)')'Number of samples in Bin ', IndexB,' :', sum(samplesInThisBin)
