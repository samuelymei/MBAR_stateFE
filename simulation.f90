module simulation_m
  ...
  public :: readUSSimulationInfo, readREMDSimulationInfo, deleteSimulationInfo
  public :: simulation_t
contains

  subroutine readUSSimulationInfo()
    ...
    integer(kind=4) :: itmp
    if(idebug == 1) write(*,*) 'Entering readUSSimulationInfo'
    allocate(simulations(nSimulations))
    do indexW = 1, nSimulations
      read(id_meta_file, *) simulations(indexW)%dataFile, simulations(indexW)%nSnapshots, simulationTemperature, &
                  & simulations(indexW)%restraintCenter, simulations(indexW)%restraintStrength
      ...
      write(6,'(A,I3,A,F10.6,A,I10,A,A)') ' Simulation ', indexW, ':    beta = ', simulations(indexW)%beta, &
         ...
         & ', Data file: ', trim(simulations(indexW)%dataFile)
  ...
  end subroutine readUSSimulationInfo

  subroutine readREMDSimulationInfo()
    use constant_m, only : kB
    use io_m
    implicit none
    real(kind=fp_kind) :: simulationTemperature
    integer(kind=4) :: indexW, indexS
    integer(kind=4) :: itmp
    if(idebug == 1) write(*,*) 'Entering readREMDSimulationInfo'
    allocate(simulations(nSimulations))
    do indexW = 1, nSimulations
      read(id_meta_file, *) simulations(indexW)%dataFile, simulations(indexW)%nSnapshots, simulationTemperature
      simulations(indexW)%beta = 1.d0 / ( kB * simulationTemperature )
    ...
    totalNumSnapshots = sum(simulations(:)%nSnapshots)
  end subroutine readREMDSimulationInfo
