...
!  call readUSSimulationInfo()
  call readREMDSimulationInfo()
...
!    call simulatedReducedHamiltonian(IndexW)%processTrajectories
    open(id_data_file, file = trim(simulations(indexW)%dataFile), status = 'old')
    call simulatedReducedHamiltonian(IndexW)%readInEnergy(id_data_file)
    close(id_data_file)
  end do
...
! (reduced)Hamiltonians and associated uncertainties
  call MBAR(2000,1.D-7,nSimulations,totalNumSnapshots,reducedEnergies,nSnapshotsInSimulation,freeEnergies, &
  ...
  do IndexW = 1, nSimulations
    write(6,'(A,I3,A,F12.2,A,F5.2,A,F8.3)')' Free energy (kcal/mol) of Hamiltonian ', IndexW, ' is: ', &
       ...
       & ' Reweighting Entropy:', reweightingEntropy(totalNumSnapshots, weights(:, IndexW))
  end do  
  do IndexW = 1, nSimulations
     write(6,'(A,I3,A,F12.2,A,F5.2)')' Shifted free energy (kcal/mol) of Hamiltonian ', IndexW, ' is: ', &
       & simulatedReducedHamiltonian(IndexW)%freeEnergy/simulatedReducedHamiltonian(IndexW)%beta - &
       & simulatedReducedHamiltonian(1)%freeEnergy/simulatedReducedHamiltonian(1)%beta, &
       & ' +- ', &
       & simulatedReducedHamiltonian(IndexW)%freeEnergySD/simulatedReducedHamiltonian(IndexW)%beta
  end do  
...
! compute weight for each sample under the target Hamiltonian
  ...
  write(6,'(A,F12.3,1X,A,F5.2,A,F8.3)')' Free energy (kcal/mol) of the target Hamiltonian is ', &
           ...
           & ' Reweighting Entropy:', reweightingEntropy(totalNumSnapshots, weights(:,nSimulations+1))
...
! delete data space
  ...
  write(6,'(10F8.3)') 'Successfully completed.'
